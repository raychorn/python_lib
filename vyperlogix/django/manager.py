...
class DjangoManager(Cooperative):
	...
	def find(self,top,fname,target=0,pos=0):
		...
		if (not os.path.exists(top)):
			sys.stderr.write('(%s)-ERROR: Cannot find %s in or under %s.\n' % (funcName(),fname,self.__top__))
			sys.exit(1)
	...
	def __init__(self,tops=[],app=None):
		...
		for drive in getCurrentWindowsLogicalDrives:
			...
			if (_foundCount):
				break
		if (_foundCount < 1):
			sys.stdout.write('WARNING: Cannot locate "%s".' % (t))
			os._exit(0)
		...
		if (app):
			...
			sys.stdout.write('WARNING: Cannot proceed without an app, as you can see "%s" is not useable.' % (app))
			os._exit(0)
		...
		if (not self.__has_vyperlogix_lib__):
			...
			if (os.path.exists(top)):
				...
				sys.stderr.write('ERROR: Cannot find %s in or under %s.' % (self.__vyperlogix_lib__,self.__top__))
				sys.exit(1)
		...
		try:
			...
			sys.stderr.write('ERROR: Cannot import any vyperlogix modules !!!')
			sys.exit(1)
